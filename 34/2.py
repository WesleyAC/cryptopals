#!/usr/bin/env python3

from Crypto.Cipher import AES
import hashlib, os, random

a_p = 
a_g = 2

a_a = random.randint(0,a_p)
a_A = pow(a_g, a_a, a_p)

# A -> M (a_p, a_g, a_A)

m_p = a_p
m_g = a_g
m_A = a_A

b_p = m_p
b_g = m_g
b_A = m_p # attack!

# M -> B (a_p, a_g, a_p)

b_b = random.randint(0,b_p)
b_B = pow(b_g, b_b, b_p)
b_s = pow(b_A, b_b, b_p)

# B -> M (b_B)

m_B = b_B

a_B = m_p # attack!

# M -> A (m_p)

a_s = pow(a_B, a_a, a_p)

a_key = hashlib.sha1(a_s.to_bytes(256, 'little')).digest()[:16]
a_iv = os.urandom(16)
a_cipher = AES.new(a_key, AES.MODE_CBC, a_iv)
a_ct = a_cipher.encrypt(b"YELLOW SUBMARINE")

# A -> M (a_ct, a_iv)

m_a_ct = a_ct
m_a_iv = a_iv

m_s = 0
m_key = hashlib.sha1(m_s.to_bytes(256, 'little')).digest()[:16]
m_a_decrypt_cipher = AES.new(m_key, AES.MODE_CBC, m_a_iv)
m_a_pt = m_a_decrypt_cipher.decrypt(m_a_ct)

print(f"M has received message: {m_a_pt}")

b_a_ct = a_ct
b_a_iv = a_iv

# M -> B (a_ct, a_iv)

b_key = hashlib.sha1(b_s.to_bytes(256, 'little')).digest()[:16]
b_decrypt_cipher = AES.new(b_key, AES.MODE_CBC, b_a_iv)
b_pt = b_decrypt_cipher.decrypt(b_a_ct)

print(f"B has received message: {b_pt}")

b_iv = os.urandom(16)
b_encrypt_cipher = AES.new(b_key, AES.MODE_CBC, b_iv)
b_ct = b_encrypt_cipher.encrypt(b_pt)

# B -> M (b_ct, b_iv)

m_b_ct = b_ct
m_b_iv = b_iv

m_b_decrypt_cipher = AES.new(m_key, AES.MODE_CBC, m_b_iv)
m_b_pt = m_b_decrypt_cipher.decrypt(m_b_ct)

print(f"M has received message: {m_b_pt}")

a_b_ct = b_ct
a_b_iv = b_iv

# M -> A (b_ct, b_iv)

a_decrypt_cipher = AES.new(a_key, AES.MODE_CBC, a_b_iv)
a_pt = a_decrypt_cipher.decrypt(a_b_ct)

print(f"A has received message: {a_pt}")
